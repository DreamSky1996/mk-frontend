...
const Merge = () => {
    ...
    const merge = async () => {
        ...
        console.log(firstIDs, secondIDs);
        await dispatch(mkMultipleMerges({ firstIdList: firstIDs, secondIdList: secondIDs, basePrice: basePrice, provider: provider, networkID: chainID }));
    }
    return (
        <div>
            <h1>Merge</h1>
            <Container>
                ...
                <br/>
                <div>
                    First Token IDs: <input value={firstList} onChange={e => setfirstList(e.target.value)}/>
                </div>
                <br/>
                <div>
                   Second Token IDs: <input  value={secondList} onChange={e => setSecondList(e.target.value)} />
                </div>
                <br/>
                <div>
