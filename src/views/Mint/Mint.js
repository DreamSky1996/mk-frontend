...
const Mint = () => {
    ...
    const onMint = async () => {
        console.log(amount);
        if ( amount == 0 ) return;
        await dispatch(mkMint({ amount: amount, mintPrice: mintPrice, provider: provider, networkID: chainID }));
    };
    return( 
        <Background>
            <MintContainer>
                <InfoContainer style={{top: '20%', left: '14.5%'}}>
                    <PriceText style={{top: '25%', left: '1%'}}>Price: {mintPrice} ETH</PriceText>
                </InfoContainer>
