...
const Stake = () => {
    ...
    const stake = async () => {
        ...
        await dispatch(mkStake({ idList: stakeIDs, provider: provider, networkID: chainID }));
    };

    const unstake = async () => {
        console.log(unStakeList);
        if ( unStakeList == "" ) return;
        let unstakeIDs = unStakeList.split(",").map(s => s.trim());;
        console.log(unstakeIDs);
        await dispatch(mkUnstake({ idList: unstakeIDs, provider: provider, networkID: chainID }));
    };
    return( 
        <div>
            <h1>Stake</h1>
            <Container>
                <div style={{textAlign: "center"}}>
                    ...
                    <div>
                        <input value={stakeList} onChange={e => setStakeList(e.target.value)}/>
                        <button onClick={stake}>Stake</button>
                    </div>
                    <br/>
                    <div>
                        <input value={unStakeList} onChange={e => setUnStakeList(e.target.value)}/>
